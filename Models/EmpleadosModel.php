...
class EmpleadosModel extends Query{
    ...
    public function insertEmpleado($datos){
        $query = "INSERT INTO empleados(primernombre, segundonombre, primerapellido, segundoapellido, dui, sexo, fechanacimiento, fechaingreso, cargo, sueldo, seguromedico, pension, numeroafiliado, cuenta_banco) 
                  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public function updateEmpleado($datos, $id){
        $query = "UPDATE empleados SET primernombre = ?, segundonombre = ?, primerapellido = ?, segundoapellido = ?, sexo = ?, fechanacimiento = ?, fechaingreso = ?, cargo = ?, seguromedico = ?, pension = ?, numeroafiliado = ?, cuenta_banco = ? WHERE id = ?";
        $datos[] = $id;
        $data = $this->save($query, $datos);
        if ($data == 1) {
            $res = "ok";
