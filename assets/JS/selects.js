...
document.addEventListener("DOMContentLoaded", function(){
    ...
    $('.planillavacaciones').select2({
        placeholder: "Buscar cambios en la planilla de vacaciones",
        theme: 'bootstrap-5',
        allowClear: true,
        minimumInputLength: 0,
        ajax: {
            url: base_url + 'Vacaciones/buscarCambios',
            dataType: 'json',
            delay: 250,
            data: function(params) {
                return { date: params.term };
            },
            processResults: function(data) {
                return { results: data };
            },
...
$('.selectgenerarPlanillaVacaciones').select2({
    dropdownParent: $('#planillaVacacionesModal'),
    theme: 'bootstrap-5',
    allowClear: true,
    minimumInputLength: 0,
    data: meses,
...
$("#selectplanillaanio").on("change", function() {
    ...
    if (anio === '') {
        ...
        $("#selectplanillames option[value]").remove();
        const url = base_url + "Vacaciones/buscar_mes/" + anio;
